<nav 
    ...
    class="flex items-center justify-between px-6 py-4 border-b bg-neutral-50 border-neutral-300 dark:border-neutral-700 dark:bg-neutral-900" aria-label="penguin ui menu"
>
	<!-- Brand Logo -->
    ...
    <div class="flex items-center gap-6"> 
        ...
        <!-- Mobile Menu -->
        <ul x-cloak x-show="mobileMenuIsOpen" x-transition:enter="transition motion-reduce:transition-none ease-out duration-300" x-transition:enter-start="-translate-y-full" x-transition:enter-end="translate-y-0" x-transition:leave="transition motion-reduce:transition-none ease-out duration-300" x-transition:leave-start="translate-y-0" x-transition:leave-end="-translate-y-full" id="mobileMenu" class="fixed inset-x-0 top-0 z-10 flex flex-col px-6 pt-20 pb-6 overflow-y-auto border-b divide-y rounded-b-sm max-h-svh divide-neutral-300 border-neutral-300 bg-neutral-50 dark:divide-neutral-700 dark:border-neutral-700 dark:bg-neutral-900 md:hidden">
            @auth
                <li class="py-4">
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
